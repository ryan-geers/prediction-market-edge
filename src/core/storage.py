    ...
    def mark_open_positions(self, marks: Iterable[PositionMark]) -> int:
        ...
        for mark in marks:
            if mark.yes_bid is not None and mark.yes_ask is not None:
                ...
                # fair_mid is only safe as a fallback when the quote assessment
                # says the mark is reliable. Signal-only proxies and snapshot
                # placeholders must not overwrite live position accounting.
                fair_mid = (
