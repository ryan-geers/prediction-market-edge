...
class Storage:
    ...
    def close_stale_positions(
        ...
        When ``position_ids`` is provided, only those already-inspected rows are
        eligible for closure.  The realized_pnl is set to the last known
        unrealized_pnl (i.e. position is closed at the last mark price).
        ...
        if position_ids is not None:
            ...
            position_filter = f" AND position_id IN ({placeholders})"
